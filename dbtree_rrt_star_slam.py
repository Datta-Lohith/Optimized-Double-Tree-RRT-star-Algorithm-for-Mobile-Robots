...
def create_map():
    args = rospy.myargv()[1:]
    if len(args) == 0:
        print("Please provide the map image as an argument")
        exit()

    map_image = cv.imread(args[0], cv.IMREAD_GRAYSCALE)
    if map_image is None:
        print("Please provide a valid image file")
        exit()

    shapes=map_image.shape
    dimensions = np.array([(0, shapes[1]), (0, shapes[0])])  # dimensions of Search Space
    print("Map dimensions:",dimensions)
    inverted_image = 255 - map_image
    # Apply thresholding to convert the grayscale map_image to binary
    _, binary_map_image = cv.threshold(inverted_image, 127, 255, cv.THRESH_BINARY)
    edges = cv.Canny(binary_map_image, 50, 150)

    # Find contours in the binary map_image
    contours, _ = cv.findContours(edges, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    Obstacles=[]
    # Draw the contours on the original grayscale map_image
    for contour in contours:
        x, y, w, h = cv.boundingRect(contour)
        Obstacles.append((x,384-y,x+w,384-y-h))
...
# Creating the map
Dimensions,Obstacles,map_image=create_map()
